...
class TopAccuracyTextsNoDuplicates:
    def __init__(self, max_size=5):
        ...
        self.text_map = (
            {}
        )  # 텍스트를 키로, (힙 내 위치, 생성 시점)을 값으로 하는 딕셔너리
    ...
    def get_top_texts(self):
        ...
        return sorted(
            [(accuracy, text, ep) for accuracy, _, text, ep in self.heap], reverse=True
        )
...
# 전체 테스트 셋에 대해서 테스트
def evaluation_full(
    prompts, imdb, model, tokenizer, device, verbalizer=["Yes", "No"], side="Last"
):
    accs = []
    ...
    for prompt in prompts:
        ...
        for i in range(len(targets)):
            ...
            with torch.no_grad():
                ...
                answer_log_probs = (
                    log_probs[0, -len(answer_tokens) - 1 : -1].sum().item()
                )
...
def language_feedback(
    ...
    for prompt, accuracy in zip(used_prompt, accuracys):
        feedback += "Instruction: {prompt}\nAccuracy: {accuracy:.2f}\n".format(
            prompt=prompt, accuracy=accuracy
        )
    feedback_query = [
        {
            "role": "user",
            "content": """This is output instruction and it's accuracy. Please describe the problems of the instruction generated to solve this problem. \nTasks : """
            + args.meta_question
            + "\n"
            + example
            + feedback,
    ...
    feedback_output = ppo_trainer.generate(
        ppo_tokenizer.apply_chat_template(feedback_query, return_tensors="pt")
        .view(-1)
        .to(device),
        **generation_kwargs,
        return_prompt=False,
    ...
    feedback_output = ppo_tokenizer.decode(
        feedback_output.squeeze(), skip_special_tokens=True
    )
    update_query = [
        {
            "role": "user",
            "content": "Please write instructions to fix this problem.\n"
            + feedback_output,
        },
        {"role": "assistant", "content": "Instruction : "},
    ]
    update_output = ppo_trainer.generate(
        ppo_tokenizer.apply_chat_template(update_query, return_tensors="pt")
        .view(-1)
        .to(device),
        **generation_kwargs,
        return_prompt=False,
    ...
    update_output = ppo_tokenizer.decode(
        update_output.squeeze(), skip_special_tokens=True
    )
    updated_query_text = [
        {
            "role": "user",
            "content": args.meta_question
            + "\n"
            + example
            + "\n Here is some tips for writing instruction :"
            + update_output,
        },
        {"role": "assistant", "content": "Instruction : "},
    ]
...
def evaluation_roberta(
    ...
    def _get_logits(texts, tokenizer, model, device):
        ...
        mask_token_indices = _get_mask_token_index(
            encoded_inputs["input_ids"], tokenizer
        )
    ...
    for prompt in prompts:
        ...
        with torch.no_grad():
            for i, batch in enumerate(dataloader):
                ...
                formatted_templates = _format_prompts(
                    current_prompts, inputs, side=side
                )
...
def create_balanced_subset_and_validation(dataset, subset_size, label_key="label"):
    ...
    while len(validation_set) < subset_size:
        ...
        validation_set.append(
            random.choice(by_label[label][len(by_label[label]) // 2 :])
        )
    ...
    for prompt in prompts:
        ...
        for idx in incorrect_indices:
            incorrect_pairs.append(
                (inputs[idx], preds[idx].item(), targets[idx].item())
            )
...
def evaluation_cot(
    ...
    def _get_answer(texts, tokenizer, model, device):
        encoded_texts = tokenizer.apply_chat_template(texts, return_tensors="pt").to(
            device
        )
        generated_output = model.generate(encoded_texts, **generation_kwargs)
        generated_output = tokenizer.decode(
            generated_output.squeeze(), skip_special_tokens=True
        )
...
def evaluation_sdX(
    ...
    # Initialize results containers for all prompts
    accuracys = [0] * len(prompts)
    sds = [0] * len(prompts)
    totals = [0] * len(prompts)
    corrects = [0] * len(prompts)
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            # Process all prompts at once for this batch
            softmax_diffs, accs = evaluation_softX(
                prompts,  # Pass the entire list of prompts
                inputs,
                targets,
                model,
                tokenizer,
            ...
            # Update metrics for each prompt
            current_batch_size = len(targets)
            for i, (softmax_diff, acc) in enumerate(zip(softmax_diffs, accs)):
                corrects[i] += acc * current_batch_size
                totals[i] += current_batch_size
                sds[i] += softmax_diff

            if debug:
                print(inputs, targets, accs)
...
def create_prompts_formatted(completions, tokenizer):
...
def evaluation_softX(
    ...
    # print("evaluateX", model.device)

    def format_prompts(prompt_list, input_list, side):
        if side == "First":
            # template = "{prompt} Input : {sentence_1} Output:"

            template_format = lambda prompt, sentence_1: tokenizer.apply_chat_template(
                [
                    {"role": "user", "content": f"{prompt} Input : {sentence_1}"},
            ...
            template_format = lambda prompt, sentence_1: tokenizer.apply_chat_template(
                [
                    {"role": "user", "content": f"{sentence_1} {prompt}"},
                    {"role": "assistant", "content": "Output:"},
                ],
                tokenize=False,
                continue_final_message=True,
        ...
        # Create a cartesian product of prompts × inputs
        formatted_templates = []
        prompt_indices = []

        for p_idx, prompt in enumerate(prompt_list):
            for input_text in input_list:
                formatted_templates.append(template_format(prompt, input_text))
                prompt_indices.append(p_idx)

        return formatted_templates, prompt_indices

    def get_next_token_index(input_ids):
        # Return the position after the last token in the input
        return input_ids.shape[1] - 1

    def get_logits(texts, tokenizer, model, device):
        batch_size = len(texts)

        # texts = create_prompts_formatted(texts, tokenizer)
        ...
        with torch.inference_mode():
            token_logits = model(**encoded_inputs.to(device)).logits
            next_token_indices = get_next_token_index(encoded_inputs["input_ids"])
            out_logits = token_logits[range(batch_size), next_token_indices, :]
    ...
    # Format all combinations of prompts and inputs at once
    formatted_templates, prompt_indices = format_prompts(prompts, inputs, side=side)

    # Process all formatted templates in one batch (or in smaller batches if too large)
    # Adjust based on your GPU memory
    all_logits_list = []

    for i in range(0, len(formatted_templates), max_batch_size):
        # print("double_inner_loop")
        batch_templates = formatted_templates[i : i + max_batch_size]
        batch_logits = get_logits(batch_templates, tokenizer, model, device)
        all_logits_list.append(batch_logits)

    all_logits = torch.cat(all_logits_list, dim=0)

    ## gather here ->
    # if hasattr(model, "gather_on_root"):
    #    preds = model.gather_on_root(preds)

    # Get verbalizer logits for all combinations
    verbalizer_logits = all_logits[:, verbalizer_ids]
    log_probs = F.softmax(verbalizer_logits, dim=1)

    # Reshape to organize by prompts and inputs
    # Shape becomes [num_prompts, batch_size, num_verbalizers]
    log_probs_reshaped = log_probs.view(num_prompts, batch_size, -1)

    # Get predictions for each prompt
    preds = torch.argmax(log_probs_reshaped, dim=2)  # [num_prompts, batch_size]

    # Calculate accuracies and rewards for each prompt
    accuracies = []
    rewards = []

    targets = targets.to(device)
    for p_idx in range(num_prompts):
        prompt_preds = preds[p_idx]
        correct_predictions = torch.sum(prompt_preds == targets)
        accuracy = correct_predictions.item() / batch_size
        accuracies.append(accuracy)

        # Extract logits for this prompt
        prompt_logits = all_logits.view(num_prompts, batch_size, -1)[p_idx]
...
def get_reward(
    ...
    reward = torch.where(
        differences > 0,
        differences * Success_coefficient,
    ...
    for i in range(shot):
        ...
        if "text" in example.keys():
            a = (
                example["text"]
                + "\nOutput : "
                + dataset_dict[example[label_key]]
                + "\n"
            )
            examples += a
        else:
...
def got_example_bbh(
    dataset, dataset_dict, shot=5, label_key="label", metrics="multiple_choice_grade"
):
    examples = ""
    for i in range(shot):
        idx = random.randint(0, len(dataset) - 1)
...
def evaluation_generation(prompts, dataset, model, tokenizer, device, show=False):
            ...
            for prompt in prompts:
                ...
                for batch in tqdm(dataloader):
                    ...
                    outputs = model.generate(
                        **prompt_encoded, max_new_tokens=10, do_sample=True
                    )
                    e_outputs = tokenizer.decode(outputs[0], skip_special_tokens=True)
                    r_outputs = tokenizer.decode(
                        outputs[0][len(prompt_encoded[0]) :], skip_special_tokens=True
                    ).replace("\n", "")
...
def simple_bleu(prediction: str, references: List[str]) -> float:
    ...
    for reference in references:
        ...
        for word in prediction_count:
            max_counts[word] = max(
                max_counts.get(word, 0), reference_count.get(word, 0)
            )

    # 클립된 정밀도 계산
    clipped_count = sum(
        min(count, max_counts[word]) for word, count in prediction_count.items()
    )
...
def _format_prompts(prompts, inputs):
    return [
        prompt + "\n" + "Input : " + inputss + "\nOutput : "
        for prompt, inputss in zip(prompts, inputs)
    ]
...
def tta_evaluation(
    ...
    for batch in tqdm(dataloader):
        ...
        input_ids = agent_tokenizer(
            meta_prompted_inputs,
            return_tensors="pt",
            padding=True,
            truncation=True,
            max_length=512,
        ...
        with torch.no_grad():
            outputs = agent_model.generate(input_ids, **generation_kwargs)
        ...
        all_logits = _get_logits_tta(
            prompted_inputs, target_tokenizer, target_model, device
        )
        verbalizer_logits = all_logits[:, verbalizer_ids]
        log_probs = F.softmax(verbalizer_logits, dim=1)
        preds = torch.argmax(log_probs, dim=1).cpu()
...
def tta_evaluation_(
    ...
    for batch in tqdm(test_dataloader):
        with torch.no_grad():
            ...
            query_encoded = (
                agent_tokenizer.apply_chat_template(query_text, return_tensors="pt")
                .view(-1)
                .to("cuda:0")
            )
