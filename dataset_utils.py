...
def load_bigbench_causal_judgment():
    with open(
        "./automatic_prompt_engineer/data/bigbench-ii/causal_judgment/task.json", "r"
    ) as f:
...
def load_bigbench_sports_understanding():
    with open(
        "./automatic_prompt_engineer/data/bigbench-ii/sports_understanding/task.json",
        "r",
    ) as f:
        json_data = json.load(f)
    text_input = []
...
def load_openbookqa():
    train_sentences = load_dataset(
        "allenai/openbookqa", "additional", split="validation"
    )
    ...
    for item in train_sentences:
        ...
        item["text"] = (
            ...
            + item["question_stem"]
            + " choices : "
            + choice_to_sentence(item["choices"]["text"])
            + "Hint :"
            + item["fact1"]
        )
...
def load_mmlu(dataset_name):
    ...
    for item in train_sentences:
        item["text"] = (
            ...
            + item["question"]
            + " choices : "
            + choice_to_sentence(item["choices"])
        )
...
def load_bigbench(dataset_name):
    with open(
        "./automatic_prompt_engineer/data/bigbench-ii/" + dataset_name + "/task.json",
        "r",
    ) as f:
        json_data = json.load(f)
...
def load_qa_econometrics():
    ...
    for item in train_sentences:
        item["text"] = (
            "question : " + item["question"] + " choices : " + str(item["choices"])
        )
        item["label"] = item["answer"]
    for item in test_sentences:
        item["text"] = (
...
def load_bigbench_snarks():
    with open(
        "./automatic_prompt_engineer/data/bigbench-ii/snarks/task.json", "r"
    ) as f:
        json_data = json.load(f)
    text_input = []
    labels = []
...
def load_bigbench_presuppositions_as_nli():
    with open(
        "./automatic_prompt_engineer/data/bigbench-ii/presuppositions_as_nli/task.json",
        "r",
    ) as f:
        json_data = json.load(f)
    text_input = []
...
def load_bigbench_implicatures():
    with open(
        "./automatic_prompt_engineer/data/bigbench-ii/implicatures/task.json", "r"
    ) as f:
        json_data = json.load(f)
    text_input = []
    labels = []
...
def load_bigbench_navigate():
    with open(
        "./automatic_prompt_engineer/data/bigbench-ii/navigate/task.json", "r"
    ) as f:
        json_data = json.load(f)
    text_input = []
    labels = []
...
def load_bigbench_epistemic_reasoning():
    with open(
        "./automatic_prompt_engineer/data/bigbench-ii/epistemic_reasoning/task.json",
        "r",
    ) as f:
        json_data = json.load(f)
    text_input = []
...
def load_customer_review():
    ...
    train_sentences = load_dataset(
        "csv", data_files=file_dict, split="train", delimiter="\t"
    )
    train_labels = train_sentences["label"]
    file_dict = {"train": "cr/16-42/test.tsv"}
    test_sentences = load_dataset(
        "csv", data_files=file_dict, split="train", delimiter="\t"
    )
    test_labels = test_sentences["label"]
    # str2int = train_sentences.features['label']._str2int
...
def dataset_dicts(dataset_name):
    if "mmlu" in dataset_name:
        ...
        return {0: "negative", 1: "positive"}  # 0:no, 1:yes
    elif dataset_name == "qnli":
        return {0: "yes", 1: "no"}
    elif dataset_name == "mnli":
...
def load_annotation(dataset):
    ...
    if dataset == "sst2":
        annotation = """
        In this task, you are given sentences from movie reviews. The task is to classify a sentence as "positive" if the sentiment of the sentence is positive or as "negative" if the sentiment of the sentence is negative.',
        """
    elif dataset == "mnli":
        annotation = """
